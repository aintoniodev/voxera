...
(defn run-helper! [script]
  ;; On Windows, bb cannot exec a .sh helper directly (no shebang support in
  ;; CreateProcess); run the sibling .bb with the bb runtime instead.
  (if (str/includes? (System/getProperty "os.name") "Windows")
    (let [bb-script (str/replace (str (fs/path script-dir script)) #"\.sh$" ".bb")]
      (process/exec "bb" bb-script))
